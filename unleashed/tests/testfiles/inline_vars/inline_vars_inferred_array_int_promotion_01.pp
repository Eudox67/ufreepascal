...
// integer literals in an inferred array literal promote to PtrInt
...
begin
  ...
  // even if all literals fit in Byte, element type stays PtrInt
  var b := [10, 200, 100];
  if Length(b) <> 3 then halt(6);
  if SizeOf(b[0]) <> SizeOf(PtrInt) then halt(7);
end.
